...
if(is.na(args$multiqc_html)){
  qc_link <- "qc/multiqc_report.html"
} else{
...
if(is.na(args$zoonotic_rank_predictions)){
  zoonotic_rank_predictions_file <- paste(prefix, "/analysis/zoonotic_rank/", prefix, ".predictions.csv", sep="")
} else{
...
render(rmd_template_file,
      output_file=paste(prefix,"_analysis_report", sep = ""),
      output_dir=prefix,
      knit_root_dir=getwd(),
